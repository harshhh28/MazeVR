        ...
        while(KP_EventsQueued(&platform)) {
            ...
            switch(e->type) {
                case KP_EVENT_KEY_PRESS:{
                    ...
                    switch(e->key) {
                        case KEY_ESCAPE:{
                            if(active_menu == MENU_MAIN) MainMenuPlay();
                            active_menu = MENU_MAIN;
                        }break;
                        case KEY_ENTER:{
...
#define AI_LERP 10.f
        ...
        for(int dodec_it = 0; dodec_it < num_dodecahedrons; ++dodec_it) {
            if(!dodecahedrons[dodec_it].active) continue;
            for(int f = 0; f < 36; ++f) {
        ...
        for(; world_it < num_world_faces; ++world_it) {
            ...
            if(world_face_rot.v0.z < NEAR_Z && world_face_rot.v1.z < NEAR_Z && world_face_rot.v2.z < NEAR_Z) continue;
            ...
            else if(num_verts_to_clip == 1) {if(verts_to_clip == 0b1) { // Clip v0
...
int main(int argc, char* argv[]) {
    ...
    if(!(
        ...
        KS_Load(&textures[15], "textures/croakingkero.png"))) {
...
#define MAX_RAY_LENGTH 850
        ...
        for(; world_it < num_world_faces; ++world_it) {
            ...
            if(world_face_rot.v0.z < NEAR_Z && world_face_rot.v1.z < NEAR_Z && world_face_rot.v2.z < NEAR_Z/* || world_face_rot.v0.z > FAR_Z && world_face_rot.v1.z > FAR_Z && world_face_rot.v2.z > FAR_Z*/) continue;
            // clip on NEAR_Z plane
            num_verts_to_clip = verts_to_clip = 0;
            if(world_face_rot.v0.z < NEAR_Z) {
                ++num_verts_to_clip;
            ...
            vec2_t uv[4] = {world_face_rot.uv[0].u, world_face_rot.uv[0].v, world_face_rot.uv[1].u, world_face_rot.uv[1].v, world_face_rot.uv[2].u, world_face_rot.uv[2].v,
            };
            if(num_verts_to_clip == 0) {
                // All verts are >= NEAR_Z. No clipping
                cam_faces[cam_it++] = world_face_rot;
